...
class PartitionTestCase(TestCase):
    ...
    def setUp(self):
        # Create a memory-based user service to use instead of the default one which persists
        self.user_service = MemoryUserService()

        # Set up two user partition schemes: random and mock
...
class TestPartitionService(PartitionTestCase):
    ...
    def test_get_user_group_id_for_partition(self):
        ...
        group1_id = self.partition_service.get_user_group_id_for_partition(self.user_partition.id)
        ...
        group2_id = self.partition_service.get_user_group_id_for_partition(self.user_partition.id)
